...
export default function Projects() {
  ...
  const renderProjectCard = (project: typeof projectsData[0], idx: number) => {
    const TagIcon = projectTagIcons[project.tag.icon as keyof typeof projectTagIcons];
      ...
      <div key={project.id}>
        <ScrollReveal delay={idx * 0.15}>
          <div
            ...
            className="group relative bg-white border-[4px] border-black p-6 sm:p-8 shadow-[8px_8px_0px_#000] rounded-lg hover:translate-x-1.5 hover:translate-y-1.5 hover:shadow-[2px_2px_0px_#000] transition-all duration-200 flex flex-col"
          >
  ...
  return (
    <section id="projects" className="py-20 bg-[#F5F1E8] select-none">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Project Cards - True Masonry Flex Layout */}
        <div className="flex flex-col lg:flex-row gap-12 items-start">
          {(() => {
            const col1: { item: typeof projectsData[0], idx: number }[] = [];
            const col2: { item: typeof projectsData[0], idx: number }[] = [];
            let h1 = 0;
            let h2 = 0;

            projectsData.forEach((project, idx) => {
              const estHeight = JSON.stringify(project).length;
              if (h1 <= h2) {
                col1.push({ item: project, idx });
                h1 += estHeight;
              } else {
                col2.push({ item: project, idx });
                h2 += estHeight;
              }
            });
            ...
            return (
              <>
                <div className="flex flex-col gap-12 flex-1 w-full">
                  {col1.map(({ item, idx }) => renderProjectCard(item, idx))}
                </div>
                <div className="flex flex-col gap-12 flex-1 w-full">
                  {col2.map(({ item, idx }) => renderProjectCard(item, idx))}
                </div>
              </>
            );
          })()}
