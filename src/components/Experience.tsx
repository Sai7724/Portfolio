...
export default function Experience() {
  const renderExperienceCard = (exp: typeof experienceData[0], idx: number) => (
    <div key={exp.id}>
  ...
  return (
    <section id="experience" className="py-20 bg-[#F5F1E8] border-y-[4px] border-black select-none">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Experience Cards - True Masonry Flex Layout */}
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          {(() => {
            const col1: { item: typeof experienceData[0], idx: number }[] = [];
            const col2: { item: typeof experienceData[0], idx: number }[] = [];
            let h1 = 0;
            let h2 = 0;

            experienceData.forEach((exp, idx) => {
              const estHeight = JSON.stringify(exp).length;
              if (h1 <= h2) {
                col1.push({ item: exp, idx });
                h1 += estHeight;
              } else {
                col2.push({ item: exp, idx });
                h2 += estHeight;
              }
            });
            ...
            return (
              <>
                <div className="flex flex-col gap-8 flex-1 w-full">
                  {col1.map(({ item, idx }) => renderExperienceCard(item, idx))}
                </div>
                <div className="flex flex-col gap-8 flex-1 w-full">
                  {col2.map(({ item, idx }) => renderExperienceCard(item, idx))}
                </div>
              </>
            );
          })()}
